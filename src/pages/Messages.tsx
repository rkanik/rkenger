...
const Messages: React.FC<Props> = ({ history }) => {
	...
	const groupConversations = $msg.conversations.filter(
		({ type }) => type === _group
	)
	const directConversations = $msg.conversations.filter(
		({ type }) => type === _direct
	)
	...
	// Methods
	const isMe = (id: string) => $auth.currentUser?.id === id
	...
	// Computed
	const msgGroups = groupify($msg.messages, 'sendBy', 'messages', [
		'sentAt',
		'sendBy',
	]).reverse()

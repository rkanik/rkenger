		...
		<Fragment>
			<div
				className={`group items-center${
					isMe ? ' flex flex-row-reverse' : ' flex'
				}`}
			>
				<p
					className={classify([
						radius,
						'inline-block p-3 text-white dark:text-gray-100 text-base rounded-xl',
				...
				>
					...
					>
						<div className="bg-white dark:bg-gray-500  rounded overflow-hidden flex flex-col text-center">
							<button className="py-2 px-4 text-sm text-gray-700 dark:text-gray-200 opacity-80 hover:opacity-100">
								Edit
							</button>
							<button
								onClick={handleDelete}
								className="py-2 px-4 text-sm text-gray-700 dark:text-gray-200 opacity-80 hover:opacity-100"
							>
								Delete
							</button>
						</div>
